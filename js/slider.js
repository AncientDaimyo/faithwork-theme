(function () {
    jQuery(document).ready(function () {
        ...
        walkthrough = {
            ...
            setBtns: function () {
                var jQuerylastBtn, jQuerynextBtn, jQueryprevBtn;
                jQuerynextBtn = jQuery('.next-screen');
                jQueryprevBtn = jQuery('.prev-screen');
                jQuerylastBtn = jQuery('.finish');
                if (walkthrough.index === walkthrough.indexMax()) {
                    jQuerynextBtn.prop('disabled', true);
                    jQueryprevBtn.prop('disabled', false);
                    return jQuerylastBtn.addClass('active').prop('disabled', false);
                } else if (walkthrough.index === 0) {
                    jQuerynextBtn.prop('disabled', false);
                    jQueryprevBtn.prop('disabled', true);
                    return jQuerylastBtn.removeClass('active').prop('disabled', true);
                } else {
                    jQuerynextBtn.prop('disabled', false);
                    jQueryprevBtn.prop('disabled', false);
                    return jQuerylastBtn.removeClass('active').prop('disabled', true);
            ...
            goTo: function (index) {
                jQuery('.screen').eq(index).addClass('active');
                return jQuery('.dot').eq(index).addClass('active');
            },
            reset: function () {
                return jQuery('.screen, .dot').removeClass('active');
            },
            indexMax: function () {
                return jQuery('.screen').length - 1;
            },
            closeModal: function () {
                jQuery('.walkthrough, .shade').removeClass('reveal');
                return setTimeout((() => {
                    jQuery('.walkthrough, .shade').removeClass('show');
                    this.index = 0;
                    return this.updateScreen();
                }), 200);
            },
            openModal: function () {
                jQuery('.walkthrough, .shade').addClass('show');
                setTimeout((() => {
                    return jQuery('.walkthrough, .shade').addClass('reveal');
                }), 200);
                return this.updateScreen();
            }
        };
        jQuery('.next-screen').click(function () {
            return walkthrough.nextScreen();
        });
        jQuery('.prev-screen').click(function () {
            return walkthrough.prevScreen();
        });
        jQuery('.close').click(function () {
            return walkthrough.closeModal();
        });
        jQuery('.open-walkthrough').click(function () {
